...
function addOrUpdateCandidate()
{
	...
	$.ajaxSetup({async:false});
	$.post("/bootcamp/addOrUpdateCandidate", {id: "" + id + "",name: "" + name + "",mobile: "" + mobile + "",email: "" + email + "",gender: "" + gender + "",
				...
				if(id == "")
	   			{
		   			$.toast({
					    ...
					    afterShown: function () {
					    	setTimeout(function() {
					    		window.location.href = "/bootcamp/candidate-list"
					    	}, 5000);
					    },
					}) 
	   			}
...
<c:choose>
		<c:when test="${source eq 'view_candidate'}">
			 <a class="btn btn-sm btn-light text-primary" href="/bootcamp/view-candidate?id=${candidate.id}">
