...
function addOrUpdateCandidate()
{
	...
	$.ajaxSetup({async:false});
	$.post("/bootcamp/client/addOrUpdateCandidate", {id: "" + id + "",name: "" + name + "",mobile: "" + mobile + "",email: "" + email + "",gender: "" + gender + "",
				...
				if(id == "")
	   			{
		   			$.toast({
					    ...
					    afterShown: function () {
					    	setTimeout(function() {
					    		window.location.href = "/bootcamp/client/candidate-list"
					    	}, 5000);
					    },
					}) 
	   			}
...
<c:choose>
		<c:when test="${source eq 'view_candidate'}">
			 <a class="btn btn-sm btn-light text-primary" href="/bootcamp/client/view-candidate?id=${candidate.id}">
